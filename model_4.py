...
for alfa_cut in alfa_cut_: 
    ...
    coefficient_of_mean_deviation = z_alfa* sum(m.ps[s]*(m.va[s]+m.vb[s]) for s in m.S)
    ...
    #1
    for s in m.S :
         m.Constraints.add(m.Cs[s] == sum(( m.Ar[j,p] + (m.c_jp[j,p]*m.Ar_hat[j,p])) * m.dd[i,j,p,s] / m.q[i,j,p,s] + (m.hr[j,p]+ m.c_jp[j,p]*m.hr_hat[j,p]) * (m.q[i,j,p,s] - m.b[i,j,p,s])**2 / (2 * m.q[i,j,p,s]) + (m.phi_hat[i,p]+ m.c_ip[i,p]*m.phi_hat_hat[i,p]) * m.b[i,j,p,s] / (2 * m.q[i,j,p,s]) + ((m.phi[i,p]+m.c_ip[i,p]*m.phi_hatt[i,p]) * m.b[i,j,p,s] * m.dd[i,j,p,s]) / m.q[i,j,p,s] for i in m.I for j in m.J for p in m.P))
    ...
    term_1 = sum((m.As[i,p] + (m.c_ip[i,p]*m.As_hat[i,p])) * pyo.value(m.dd[i,j,p,s]) / pyo.value(m.q[i,j,p,s]) for s in m.S for i in m.I for j in m.J for p in m.P )
    term_2 = pyo.value(m.obj)
    z_novmi = term_1 + term_2
    objective_values.append(z_novmi)
